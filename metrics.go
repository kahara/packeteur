...
func setupMetrics(relay bool, collect bool) {
	if relay && !collect { // Capture and send packets
		...
		captured_bytes_metric = promauto.NewHistogramVec(prometheus.HistogramOpts{
			...
			Subsystem: "capture",
			Name:      "bytes",
			Buckets:   []float64{32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536},
		}, []string{"address_family"})
	...
	} else if !relay && collect { // Collect packets
		collected_total_metric = promauto.NewCounterVec(prometheus.CounterOpts{
			Namespace: Namespace,
			Subsystem: "collect",
			Name:      "total",
		}, []string{})
	} else if relay && collect { // Collect and re-relay
		// https://github.com/kahara/packeteur/issues/2
		panic("Re-collecting not implemented, see https://github.com/kahara/packeteur/issues/2")
	}
}
